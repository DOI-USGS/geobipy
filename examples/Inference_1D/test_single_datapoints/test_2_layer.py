...
par = StatArray(np.r_[0.1, 0.001], "Conductivity", "$\frac{S}{m}$")
...
dp.additive_error = np.maximum(np.r_[1e-15, 1e-15], dp.secondary_field[np.r_[dp.channels_per_system[0]-2, -2]])
dp.additive_error = dp.secondary_field[np.r_[dp.channels_per_system[0]-2, -2]]
...
kwargs['solve_value'] = True
kwargs['value_weight'] = 1.0
