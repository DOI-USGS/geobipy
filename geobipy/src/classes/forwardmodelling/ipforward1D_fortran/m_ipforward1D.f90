module ipforward1D
    ...
    !====================================================================!
    subroutine forward1D(nFreq, nLayers, tid, freqs, tHeight, rHeight, moments, rx, separation, scale, coleCole, thkIn, predicted)
    ...
    !====================================================================!
        ...
        real(kind=8), intent(in) :: coleCole(4, nLayers)
        !f2py intent(in) ::  coleCole
        ...
        complex(kind=8) :: par(nFreq, nLayers+1)
        real(kind=8) :: thk(nLayers+1)
        ...
        call initModel(coleCole, thkIn, nLayers, nFreq, freqs, par, thk)
    ...
    !====================================================================!
    !====================================================================!
    subroutine sensitivity1D(nFreq, nLayers, tid, freqs, tHeight, rHeight, moments, rx, separation, scale, coleCole, thkIn, sens)
    ...
    !====================================================================!
        ...
        complex(kind=8), intent(in) :: par(nFreq, nLayers+1)
    ...
    !====================================================================!
        ...
        do k = 1, nFreq
            p = par(k, nL1)
            do j = 1, nC
                sens(j, k, nLayers) = p / (2.d0 * un(j, k, nL1))
            enddo
        enddo
    ...
    !====================================================================!
        ...
        complex(kind=8), intent(in) :: par(nFreq, nL1)
        complex(kind=8), intent(inout) :: rTE(nC, nFreq)
        complex(kind=8), intent(inout) :: u0(nC, nFreq)
    ...
    !====================================================================!
    !====================================================================!
    subroutine initModel(coleCole, thkIn, nLayers, nFreq, frequencies, complexResistivity, thk)
    !====================================================================!
        integer :: nLayers ! Number of layers
        integer :: nFreq
        real(kind=8), intent(in) :: coleCole(4, nLayers) ! Parameter values
        real(kind=8), intent(in) :: thkIn(nLayers) ! Thicknesses
        real(kind=8), intent(in) :: frequencies(nFreq) ! measurement frequencies
        ! real(kind=8), intent(in) :: chimIn(nLayers) ! Magnetic susceptibility
        complex(kind=8), intent(inout) :: complexResistivity(nFreq, nLayers+1)
        real(kind=8), intent(inout) :: thk(nLayers+1)
        ! real(kind=8), intent(inout) :: chim(nLayers+1)
        real(kind=8) :: omega(nFreq)
    
        integer :: i
        real(kind=8) :: DCresistivity
        real(kind=8) :: chargeability
        real(kind=8) :: relaxationTime
        real(kind=8) :: exponent
            
        complexResistivity(:, 1) = cmplx(0.d0, 0.d0, kind=8)
        omega = pi2 * frequencies
        do i = 1, nLayers
            DCresistivity = coleCole(1, i)
            chargeability = coleCole(2, i)
            relaxationTime = coleCole(3, i)
            exponent = coleCole(4, i)
            complexResistivity(:, i+1) = DCresistivity * (1.d0 - chargeability * (1.d0 - (1.d0 / (cmplx(1.d0, omega * relaxationTime, kind=8)**exponent))))
        enddo
    
        ! chim(1) = 0.d0 ; chim(2:nL1) = chimIn
        thk(1) = NaN ; thk(2:nLayers+1) = thkIn
