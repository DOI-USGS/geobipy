...
class Histogram3D(RectilinearMesh3D):
    ...
    def __getitem__(self, slic):
        ...
        if len(axis) == 0:
            out = Histogram3D(xBins=self._x.cellEdges[slic[2]], yBins=self._y.cellEdges[slic[1]], zBins=self._z.cellEdges[slic[0]])
        ...
        if len(axis) == 1:
            ...
            out = Histogram2D(xBins=self.axis(a[1]).cellEdges[slic[a[1]]], yBins=self.axis(a[0]).cellEdges[slic[a[0]]])
            out._counts += self.counts[slic0]

        else:
            a = [x for x in (0, 1, 2) if not x in axis][0]
            out = Histogram1D(bins=self.axis(a).cellEdges[slic[a]])
